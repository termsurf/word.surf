...
import { BREAKPOINTS_3_1 } from '~/lib/shared/constants/breakpoints'
  ...
  return (
    <header className="mt-64">
      <H1 className="flex flex-col !mb-16">
        ...
        <NextLink
          href="/fonts"
          className="block lowercase text-sm text-zinc-400 hover:text-violet-400 transition-colors"
        >
          Fonts
        </NextLink>
      </H1>
...
function FontLink({
  ...
  if (disabled) {
    return (
      <div
        className={clsx(
          className,
          'shadow-small1 flex flex-col gap-16 bg-zinc-100 text-left p-16 pb-32 h-full leading-content rounded-sm w-full',
        )}
      >
        <T className="lowercase block font-semibold text-h4 leading-content text-zinc-300">
          {name}
        </T>
        {sample && (
          <Grid
            // script={script}
            className="text-h1 text-zinc-800 transition-colors"
            minWidth={48}
            gap={8}
            rowGap={24}
            maxColumns={6}
  ...
  return (
    <NextLink
      ...
      className={clsx(
        className,
        'shadow-small1 hover:shadow-small2 flex flex-col gap-16 bg-zinc-50 [&_span]:hover:text-violet-600 [&_span]:transition-colors transition-all duration-200 text-left p-16 pb-32 h-full leading-content rounded-sm w-full [&_i]:hover:text-violet-400',
      )}
    >
